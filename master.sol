...
contract ApproverRole {
    ...
    constructor() internal {
        ...
        firstSignAddress = 0x4452d386D6a3F0BE606e9d567b1e2DDE73bd93A8; // You should change this address to your first sign address
        secondSignAddress = 0x5269Cd39d00fD91562fA423Be63f6826458a3B6B; // You should change this address to your second sign address
    }
...
contract PlayPadStakeMaster is Ownable, ReentrancyGuard {
    ...
    function createIDO(
        ...
        MainPlayPadContract newStakeContract = new MainPlayPadContract(
         _stakingToken,
         _poolToken,
         _startBlock,
         _finishBlock,
        ...
        newStakeContract.transferOwnership(msg.sender);
        newStakePool.push(address(newStakeContract)); // Adding All IDOs
    }
    ...
    Pausable
{
    ...
    MainPlayPadContract t_stakeContract;
    ...
    function setStakeContract(address _address) public {
        t_stakeContract = MainPlayPadContract(_address);
    }

    function setPenaltyDetails(
        bool _isPenalty,
        uint256 _penaltyRate,
        address _penaltyAddress,
        uint256 _penaltyBlockLength
    ...
    function transferTokensToNewContract() external nonReentrant {
        require(block.number > finishBlock, "try later");
        UserInfo storage userData = userInfo[msg.sender];
        uint256 pending = transferPendingReward(userData);
        uint256 totalAmountToTransfer = 0;
        for (uint256 index = 0; index < userData.userPoolIds.length; index++) {
            //attention for gas spending
            UserPoolInfo storage _poolInfo = userPoolInfo[
                userData.userPoolIds[index]
            ];
            totalAmountToTransfer = totalAmountToTransfer.add(_poolInfo.amount);
            _poolInfo.amount = 0;
        }

        IERC20 newStakingContractStakeToken = t_stakeContract.stakingToken();
        if (stakingToken == newStakingContractStakeToken) {
            totalAmountToTransfer = totalAmountToTransfer.add(pending);
            t_stakeContract.stakeTokens(totalAmountToTransfer);
            userData.amount = 0;
        } else {
    ...
    function changePrizeLimit(uint256 _amountToStake)
        external
        nonReentrant
        onlyApprover
    {
    ...
    function stakeTokens(uint256 _amountToStake)
        external
        nonReentrant
        whenNotPaused
    {
        updatePool();
        uint256 pending = 0;
        uint256 randomPoolId = uint256(
            keccak256(
                abi.encodePacked(msg.sender, now, block.number, _amountToStake)
            )
        );
        ...
        if (_amountToStake > 0) {
            ...
            if (
                user.amount.add(_amountToStake) > limitForPrize &&
